...
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(`
	select u.id, u.nome, u.nick, u.email, u.criadoEm from usuarios u inner join seguidores s
	   on u.id = s.usuario_id where s.seguidor_id = ?`, usuarioID)
	if erro != nil {
		return nil, erro
	}
	...
	for linhas.Next() {
		...
		if erro = linhas.Scan(&seguindo.ID,
			&seguindo.Nome,
			&seguindo.Nick,
			&seguindo.Email,
			&seguindo.CriadoEm); erro != nil {
			return nil, erro
		}
...
func (repositorio Usuarios) AtualizarSenha(usuarioID uint64, senhaNova string) error {
	...
	_, erro = statement.Exec(senhaNova, usuarioID)
	if erro != nil {
		return erro
	}
