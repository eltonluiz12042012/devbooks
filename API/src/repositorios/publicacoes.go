...
func (repositorio Publicacoes) BuscarPublicacoesPorUsuario(usuarioID uint64) ([]modelos.Publicacao, error) {
	linhas, erro := repositorio.db.Query(`
		select p.*, u.nick from usuarios u inner join publicacoes p
...
func (repositorio Publicacoes) Curtir(publicacaoID uint64) error {
	statement, erro := repositorio.db.Prepare(`
	 update publicaoes set curtidas = curtidas + 1 where id = ?
	`)
	if erro != nil {
		return erro
	}
...
func (repositorio Publicacoes) Descurtir(publicacaoID uint64) error {
	statement, erro := repositorio.db.Prepare(`
		update publicacoes set curtirdas = 
		CASE WHEN curtidas > 0 THEN curtidade - 1
		ELSE 0 END
		where id = ?
	`)
	if erro != nil {
